...
Description: 
    All images in the dataset are linear images in RAW format each with a MCC chart which provides an estimation of the illuminant. 
    Images are masked and provided with the MCC data only during training and testing time to prevent the cnn from using them as visual cues. 
...
from utils import normalize, linear_to_nonlinear, bgr_to_rgb, correct
...
BASE_PATH = os.path.join(DATASET_PATH, "preprocessed")
PATH_TO_IMAGES = os.path.join(DATASET_PATH,"images")
PATH_TO_COORDINATES = os.path.join(DATASET_PATH,"coordinates")
PATH_TO_CC_METADATA = os.path.join(DATASET_PATH,"metadata.txt")
...
def load_image(file_name: str) -> np.ndarray:
    ...
    loads image from image directory. 
