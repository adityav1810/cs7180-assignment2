...
Description: 
    Contains setup and helper functions which are used throughout the project
...
#set device type throughout the project
...
# Set seed values to be the same in the entire program.
...
def correct(img: Image, illuminant: Tensor) -> Image:
    ...
    # print(f"Image Device : {img.get_device()}")
    illuminant = illuminant.to(DEVICE)
    # print(f"illuminant device : {illuminant.to(DEVICE).get_device()}")
    
    

    # Correct the image
    correction = illuminant.unsqueeze(2).unsqueeze(3) * torch.sqrt(Tensor([3])).to(DEVICE)
    corrected_img = torch.div(img, correction + 1e-10).to(DEVICE)
    # print(f"correction : {correction.get_device()}")
    # print(f"corrected_img : {corrected_img.get_device()}")
    ...
    normalized_img = torch.div(corrected_img, max_img).to(DEVICE)
    # print(f"normalized_img: {normalized_img.get_device()}")
