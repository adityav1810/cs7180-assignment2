...
Description: 
    Code to train the model present in model.py.
    This script defines the training routine for a machine learning model. 
    It loads the dataset, initializes the model, sets up the loss function and optimizer,
    and trains the model using the specified training parameters.
...
from utils import print_metrics, log_metrics,DEVICE, USE_CONFIDENCE_WEIGHTED_POOLING, make_deterministic
from modules.Evaluator import Evaluator
from modules.Loss import LossTracker
...
def main():
    fold_num, epochs, batch_size, lr = 0,2000,1,0.0003
    ...
    for epoch in range(epochs):
        # print(f"Runing epoch num : {epoch}")
        ...
        if 0 < val_loss.avg < best_val_loss:
            ...
            torch.save(model, 'modelWAttention.pth')
