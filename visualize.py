...
Description: 
    Code to visualize train / test images the model predicts .
...
from utils import correct, rescale, scale,DEVICE
from ColorCheckerDataset import ColorCheckerDataset
from model2_0 import ModelFC4
...
def main():
    evaluator = Evaluator()
    # model = ModelFC4()
    model = torch.load("modelWAttention.pth")
    os.makedirs(PATH_TO_SAVED)

    for num_fold in range(NUM_FOLDS):
        test_set = ColorCheckerDataset(train=False, folds_num=num_fold)
        dataloader = DataLoader(test_set, batch_size=1, shuffle=False, num_workers=4)

        # path_to_pretrained = os.path.join("trained_models", "fc4_cwp", "fold_{}".format(0))
        # path_to_pretrained = "/home/varshney.ad/cs7180/ImageEnhancement/trained_models/trained_models/fc4_cwp/fold_0"
        # model.load("modelWAttention.pth")
        ...
        with torch.no_grad():
            for i, (img, label, file_name) in enumerate(dataloader):
                ...
                scaled_confidence = rescale(confidence, size).squeeze(0).permute(1, 2, 0).to(DEVICE)

                weighted_est = scale(rgb * confidence)
                scaled_weighted_est = rescale(weighted_est, size).squeeze().permute(1, 2, 0)

                masked_original = scale(F.to_tensor(original).permute(1, 2, 0).to(DEVICE) * scaled_confidence)
                ...
                axs[0, 1].imshow(masked_original.cpu(), cmap="gray")
